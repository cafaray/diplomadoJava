...
public final class Vista {
	...
	private static final String OPCION_SALIDA = "- para salir de la aplicación presiona X           -".concat(SALTO_LINEA);
	private static final String OPCION_REGISTRO = "- 1 Registrar usuario                              -".concat(SALTO_LINEA);
	private static final String OPCION_DESACTIVAR = "- 3 Desactivar usuario                             -".concat(SALTO_LINEA);
	private static final String OPCION_CAMBIA_CONTRASENIA = "- 2 CAMBIAR CONTRASEÑA                             -".concat(SALTO_LINEA);
	
	private static final String DESPEDIDA = "- Gracias por usar PixUp-Console                   -".concat(SALTO_LINEA);
	private static final String SELECCION_MENU = "- Selecciona una opción de menú                    -".concat(SALTO_LINEA);
	private static final String SELECCION_MENU_NO_VALIDO = "- La opción que seleccionaste no es valida         -".concat(SALTO_LINEA);
	...
	private void registrarUsuario() {
		...
		List<String> listaPreguntaSecreta = catalogos.listarPreguntaSecreta();
		...
		List<String> listaTipoUsuario = catalogos.listarTipoUsuario();
		...
		if(usuarioWrapper.getIdentificador()>0){
			out.write("Usuario registrado correctamente con identificador: ".concat(String.valueOf(usuarioWrapper.getIdentificador())));
			out.flush();
		}
